...
$onlyIf[$getGuildVar[app_tiktok;$guildID;main]==on;{newEmbed:{color:#f1090b}{description:Подача заявок на эту должность временно недоступно, либо заявка закрыта.}{author:Ошибка:$get[error.icon]}{timestamp}}{ephemeral}{interaction}]
...
$editMessage[$interactionData[message.id];{newEmbed:{author:$username[$getMessageVar[user;$interactionData[message.id];main]]:$userAvatar[$getMessageVar[user;$interactionData[message.id];main]]}{title:Заявка на Хелпера}{description:Заявка была подана участником **$username[$getMessageVar[user;$interactionData[message.id];main]]** (<@$getMessageVar[user;$interactionData[message.id];main]>) .}{field:Статус заявки:\`\`\`Принята\`\`\`}{field:Рассмотрел:**$username[$authorID]** (<@$authorID>)}{field:Данные участника:ФИО#COLON# _$getMessageVar[fio;$interactionData[message.id]]_\nВозраст#COLON# _$getMessageVar[years;$interactionData[message.id]]_\nVK#COLON# _$getMessageVar[vk;$interactionData[message.id]]_\nО себе#COLON# $getMessageVar[bio;$interactionData[message.id]]}{field:Документ подтверждающий возраст:Ниже прикреплено фото документа которое участник указал в анкете.}{image:$getMessageVar[document;$interactionData[message.id]]}{footer:ID заявки#COLON# $messageID}{thumbnail:$userAvatar[$getMessageVar[user;$interactionData[message.id];main]]}}{actionRow:{button:Принять:success:helper-accept:true}{button:Отклонить:danger:helper-reject:true}};$channelID]
$sendDm[{newEmbed:{description:Здравствуйте **$username[$getMessageVar[user;$interactionData[message.id];main]]**, мы рассмотрели заявку на Хелпера которую вы недавно подавали!}{field:Результат:Ваша заявка принита}{field:Внимание:- #Перед тем как продолжить нам необходимо убедится что вы всё ещё хотите встать на пост хелпера, нажмите на кнопку 'Подтвердить' чтобы продолжить или же 'Отмена' если вы передумали.}{timestamp}{footer:StarWorld Notify:$guildIcon}}{actionRow:{button:Подтвердить:primary:confirm-helper:false}{button:Отмена:danger:no-helper:false}};$getMessageVar[user;$interactionData[message.id];main];false]`
    },
    {
        name: "helper-reject",
        type: "interaction",
...
$editMessage[$interactionData[message.id];{newEmbed:{author:$username[$getMessageVar[user;$interactionData[message.id];main]]:$userAvatar[$getMessageVar[user;$interactionData[message.id];main]]}{title:Заявка на Хелпера}{description:Заявка была подана участником **$username[$getMessageVar[user;$interactionData[message.id];main]]** (<@$getMessageVar[user;$interactionData[message.id];main]>) .}{field:Статус заявки:\`\`\`Отклонена\`\`\`}{field:Рассмотрел:**$username[$authorID]** (<@$authorID>)}{field:Данные участника:ФИО#COLON# _$getMessageVar[fio;$interactionData[message.id]]_\nВозраст#COLON# _$getMessageVar[years;$interactionData[message.id]]_\nVK#COLON# _$getMessageVar[vk;$interactionData[message.id]]_\nО себе#COLON# $getMessageVar[bio;$interactionData[message.id]]}{field:Документ подтверждающий возраст:Ниже прикреплено фото документа которое участник указал в анкете.}{image:$getMessageVar[document;$interactionData[message.id]]}{footer:ID заявки#COLON# $messageID}{thumbnail:$userAvatar[$getMessageVar[user;$interactionData[message.id];main]]}}{actionRow:{button:Принять:success:helper-accept:true}{button:Отклонить:danger:helper-reject:true}};$channelID]
$sendDm[{newEmbed:{description:Здравствуйте **$username[$getMessageVar[user;$interactionData[message.id];main]]**, мы рассмотрели заявку на Хелпера которую вы недавно подавали!}{field:Результат:Ваша заявка отклонена}{field:Не расстраивайся!:-# У тебя ещё будут шансы, через месяц можешь попробовать подать заявку повторно.}{timestamp}{footer:StarWorld Notify:$guildIcon}};$getMessageVar[user;$interactionData[message.id];main];false]`
    },
         {
        name: "yt_appmodal",
        type: "interaction",
...
$editMessage[$interactionData[message.id];{newEmbed:{author:$username[$getMessageVar[user;$interactionData[message.id]]]:$userAvatar[$getMessageVar[user;$interactionData[message.id]]]}{title:Новая заявка на Хелпера}{description:Только что участник **$username[$getMessageVar[user;$interactionData[message.id]]]** (<@$getMessageVar[user;$interactionData[message.id]]>) подал заявку.}{field:Статус заявки:\`\`\`Отклонена\`\`\`}{field:Данные участника:ФИО#COLON# _$getMessageVar[fio;$interactionData[message.id]]_\nВозраст#COLON# _$getMessageVar[years;$interactionData[message.id]]_\nVK#COLON# _$getMessageVar[vk;$interactionData[message.id]]_\nО себе#COLON# $getMessageVar[bio;$interactionData[message.id]]}{field:Документ подтверждающий возраст:Ниже прикреплено фото документа которое участник указал в анкете.}{image:$getMessageVar[document;$interactionData[message.id]]}{footer:ID заявки#COLON# $messageID}{thumbnail:$userAvatar[$getMessageVar[user;$interactionData[message.id]]]}}{actionRow:{button:Принять:success:helper-accept:true}{button:Отклонить:danger:helper-reject:true}};$channelID]`
    },
    {
        ...
        prototype: "button",
        code: `
$interactionUpdate[{newEmbed:{author:Подтверждено}{description:Здравствуйте **$username[$authorID]**, мы рассмотрели заявку на Хелпера которую вы недавно подавали!}{field:Результат:Ваша заявка принита}{field:Внимание:-# Эта заявка было подтверждена, ваш проверяющий тот кто принял заявку.}{timestamp}{footer:StarWorld Notify:$guildIcon[1162783251241369640]}}{actionRow:{button:Подтвердить:primary:confirm-helper:true}{button:Отмена:danger:no-helper:true}}]
$giveRole[1162783251241369640;$authorID;1164568875308945541;Участник теперь стажёр.]`
    },
    {
        name: "no-helper",
        type: "interaction",
        prototype: "button",
        code: `$interactionUpdate[{newEmbed:{author:Отмена}{description:Здравствуйте **$username[$authorID]**, мы рассмотрели заявку на Хелпера которую вы недавно подавали!}{field:Результат:Ваша заявка принита}{field:Внимание:-# Эта заявка было отменена.}{timestamp}{footer:StarWorld Notify:$guildIcon[1162783251241369640]}}{actionRow:{button:Подтвердить:primary:confirm-helper:true}{button:Отмена:danger:no-helper:true}}]`
    }
];