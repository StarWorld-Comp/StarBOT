...
$endif
$onlyIf[$isUserDmEnabled[$authorID]==true;{newEmbed:{color:#f1090b}{description:Перед подачей заявки включите личные сообщения от участников сервера в настройках.}{author:Ошибка:$get[error.icon]}{timestamp}}{ephemeral}{interaction}]
$onlyIf[$getGuildVar[application_tiktok]==on;{newEmbed:{color:#f1090b}{description:Подача заявок на эту должность временно недоступно, либо заявка закрыта.}{author:Ошибка:$get[error.icon]}{timestamp}}{ephemeral}{interaction}]
$suppressErrors[{newEmbed:{color:#f1090b}{description:Не удалось обработать запрос, возможно в системе заявок проходят технические работы, либо произошла критическая ошибка.}{author:Ошибка:$get[error.icon]}{timestamp}}{ephemeral}{interaction}]
$let[error.icon;https://cdn.discordapp.com/attachments/1162658570609901641/1244579676584935465/776404508515368972.png?ex=6655a0a6&is=66544f26&hm=a068d0186245402f33b93a145dc53178d854e0b9eeec437571f0110a56038c59&]`
   },
   {
...
$setMessageVar[vk;$textInputValue[vk_url];$get[msg.id]]
...
$let[error.icon;https://cdn.discordapp.com/attachments/1162658570609901641/1244579676584935465/776404508515368972.png?ex=6655a0a6&is=66544f26&hm=a068d0186245402f33b93a145dc53178d854e0b9eeec437571f0110a56038c59&]`
  },
  {
      ...
      type: "interaction",
      prototype: "button",
      code: `$interactionReply[{newEmbed:{description:Заявка участника **$username[$interactionData[author.id]]** (<@$interactionData[author.id]>) принята администратором **$username[$authorID]** (<@$authorID>)}{footer:StarWorld Safety:$guildIcon}{timestamp}}]
$editMessage[$interactionData[message.id];{newEmbed:{author:$username[$interactionData[author.id]]]:$userAvatar[$getMessageVar[user;$interactionData[message.id]]]}{title:Новая заявка на Хелпера}{description:Только что участник **$username[$getMessageVar[user;$interactionData[message.id]]]** (<@$getMessageVar[user;$interactionData[message.id]]>) подал заявку.}{field:Статус заявки:\`\`\`Принята\`\`\`}{field:Данные участника:ФИО#COLON# _$getMessageVar[fio;$interactionData[message.id]]_\nВозраст#COLON# _$getMessageVar[years;$interactionData[message.id]]_\nVK#COLON# _$getMessageVar[vk;$interactionData[message.id]]_\nО себе#COLON# $getMessageVar[bio;$interactionData[message.id]]}{field:Документ подтверждающий возраст:Ниже прикреплено фото документа которое участник указал в анкете.}{image:$getMessageVar[document;$interactionData[message.id]]}{footer:ID заявки#COLON# $messageID}{thumbnail:$userAvatar[$interactionData[author.id]]]}}{actionRow:{button:Принять:success:helper-accept:true}{button:Отклонить:danger:helper-reject:true}};$channelID]`
  },
  {
      name: "helper-reject",
      type: "interaction",
      prototype: "button",
      code: `$interactionReply[{newEmbed:{description:Заявка участника **$username[$interactionData[author.id]]** (<@$interactionData[author.id]>) отклонена администратором **$username[$authorID]** (<@$authorID>)}{footer:StarWorld Safety:$guildIcon}{timestamp}}]
$editMessage[$interactionData[message.id];{newEmbed:{author:$username[$interactionData[author.id]]]:$userAvatar[$interactionData[author.id]]}{title:Новая заявка на Хелпера}{description:Только что участник **$username[$interactionData[author.id]]]** (<@$interactionData[author.id]]>) подал заявку.}{field:Статус заявки:\`\`\`Отклонена\`\`\`}{field:Данные участника:ФИО#COLON# _$getMessageVar[fio;$interactionData[message.id]]_\nВозраст#COLON# _$getMessageVar[years;$interactionData[message.id]]_\nVK#COLON# _$getMessageVar[vk;$interactionData[message.id]]_\nО себе#COLON# $getMessageVar[bio;$interactionData[message.id]]}{field:Документ подтверждающий возраст:Ниже прикреплено фото документа которое участник указал в анкете.}{image:$getMessageVar[document;$interactionData[message.id]]}{footer:ID заявки#COLON# $messageID}{thumbnail:$userAvatar[$interactionData[author.id]]}}{actionRow:{button:Принять:success:helper-accept:true}{button:Отклонить:danger:helper-reject:true}};$channelID]`
  },
       {
      name: "yt_appmodal",
      type: "interaction",
